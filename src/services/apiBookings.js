...
export async function getBookings({ filter, sortBy, currentPage }) {
  ...
  if (sortBy) query = query.order(sortBy.value, { ascending: sortBy.direction === "asc" });

  if (currentPage) query = query.range((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE - 1);
